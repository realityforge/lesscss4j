...
module Buildr
  module Antlr
    class << self
      ...
      def version=(version)
        raise "Invalid version #{version}" unless [3, 4].include?(version)
        @version = version
      end

      # The specs for requirements
      def runtime_dependencies(version = Buildr::Antlr.version)
        if 4 == version
      ...
      # The specs for requirements
      def dependencies(version = Buildr::Antlr.version)
        if 4 == version
          [
          ...
          [
            'org.antlr:antlr:jar:3.2', 'org.antlr:stringtemplate:jar:3.2', 'antlr:antlr:jar:2.7.7'
          ] + runtime_dependencies(version)
        end
      end
      ...
      def antlr(*args)
        ...
        rake_check_options options, :directory, :package, :version

        cp = Buildr.artifacts(self.dependencies).each(&:invoke).map(&:to_s)

        version = options[:version] || Buildr::Antlr.version

        command = version == 3 ? 'org.antlr.Tool' : 'org.antlr.v4.Tool'

        Java::Commands.java command, *(args + [{:classpath => cp, :verbose => true, :dir => options[:directory]}])
      end
    end
    ...
    def compile_antlr(input_file, *args)
      ...
      file(target_dir => input_file) do |task|
        ...
        args << input_file
        begin
          Antlr.antlr(args, :directory => target_dir, :version => options[:version])
        rescue Exception => e
          rm_rf target_dir
          raise e
        end
